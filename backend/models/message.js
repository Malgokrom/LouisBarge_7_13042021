...
exports.recupMessages = (callback) => {
    db.query(`
        SELECT 0 AS show_comments, 0 AS post_comment, 0 AS num, p.id, p.id_membres, DATE_FORMAT(p.date_post, '%d/%m/%Y %H:%i:%s') AS date_post, p.message, p.image, m.nom, m.prenom, m.avatar
        FROM Posts AS p
